...
window.addEventListener('DOMContentLoaded', () => {
     ...
     const dontShowCheckbox = document.getElementById('dontshowagain_check');

     const dontShowWelcome = localStorage.getItem("dontShowWelcome");
     if (dontShowWelcome == null || dontShowWelcome == false){
          koszontDialog.show();
     }
     ...
     const kDataToLoad = localStorage.getItem("kDataToLoad");
     if (kDataToLoad != null) {
          importJson(kDataToLoad);
     }
     ...
     karValasztMenu.addEventListener('close-menu', (event) => {
          ...
          localStorage.setItem("kDataToLoad",knev);
     });
     ...
     function calcPayment(odijJsonData, kindex) {
          ...
          if (odijJsonData == null) return; //Ha még nincs betöltve ösztöndíj adat, akkor nem csinál semmit, nem biztos, hogy kell

          //Ezt is ilyen formában csak ezzel az ösztöndíj fájlal működik
          if (kindex >= 4.13) {
               let oszdij = null;

               const searchIndex = String(kindex).replace('.', ',');

               odijJsonData.forEach(row => {
                    if (row['kreditindex'] == searchIndex) {
